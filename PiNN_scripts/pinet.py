...
class IPLayer(tf.keras.layers.Layer):
    ...
    def call(self, tensors):
        ...
        # print("ind_2 shape:", ind_2.shape)
        # print("inter shape:", inter.shape)

        # print("n_atoms (IPLayer):", n_atoms)
        # print("ind_2 values (IPLayer):", ind_2.numpy())
        # unique_indices = np.unique(ind_2)
        # print("Unique indices:", unique_indices)
        # print("Number of unique indices:", len(unique_indices))
        # missing_index = set(range(300)) - set(unique_indices)
        # print("Missing index:", missing_index)

        prop = tf.math.unsorted_segment_sum(inter, ind_2[:, 0], n_atoms)

        # print("prop shape after unsorted_segment_sum:", prop.shape)
...
class GCBlock(tf.keras.layers.Layer):
    ...
    def call(self, tensors):
        ...
        # print("n_atoms (GCBlock):", n_atoms)

        # print(f"prop shape before pp_layer: {prop.shape}")
        prop = self.pp_layer(prop)
        # print(f"prop shape after pp_layer: {prop.shape}")
        
        # print(f"inter shape before pi_layer: {prop.shape}")
        inter = self.pi_layer([ind_2, prop, basis])
        # print(f"inter shape after pi_layer: {inter.shape}")
        
        # print(f"inter shape before ii_layer: {inter.shape}")
        inter = self.ii_layer(inter)
        # print(f"inter shape after ii_layer: {inter.shape}")
        
        # print(f"prop shape before ip_layer: {prop.shape}")
        prop = self.ip_layer([ind_2, inter])
        # print(f"prop shape after ip_layer: {prop.shape}")
...
class ResUpdate(tf.keras.layers.Layer):
    ...
    def call(self, tensors):
        old, new = tensors
        # print("Shape of 'old' tensor:", old.shape)
        # print("Shape of 'new' tensor:", new.shape)
    ...
    def call(self, tensors):
        ...
        for k in ['elems', 'dist']:
            if k in tensors.keys():
                if tf.shape(tensors[k])[0] != tf.shape(tensors[k])[-1]:
                    print(f"Shape mismatch in {k}: {tf.shape(tensors[k])}")
                tensors[k] = tf.reshape(tensors[k], tf.shape(tensors[k])[:1])

        # Check input tensors before processing
        # print("=== Input tensors ===")
        ...
        # Assert and print output tensors after processing
        # print("=== Output tensors ===")
        # for key in ['ind_1', 'ind_2', 'prop', 'dist']:
        #     assert key in tensors, f"{key} not found in tensors"
...
#             tensors['prop'] = tf.cast(
#                 self.embed(tensors['elems']), tensors['coord'].dtype)
#         print(f"tensors['ind_1'] shape: {tensors['ind_1'].shape}")
#         print(f"tensors['ind_2'] shape: {tensors['ind_2'].shape}")
#         print(f"tensors['prop'] shape: {tensors['prop'].shape}")
#         print(f"tensors['dist'] shape: {tensors['dist'].shape}")
#         return tensors
...
class PiNet(tf.keras.Model):
    ...
    def train_step(self, tensors):
        with tf.GradientTape() as tape:
            ...
            # tf.debugging.assert_shapes([(pred, (nbatch, self.out_layers[0].out_units))], message=f"pred shape is incorrect")
            loss = self.compiled_loss(tensors['e_data'], pred, regularization_losses=self.losses)

        trainable_vars = self.trainable_variables
        gradients = tape.gradient(loss, trainable_vars)
        self.optimizer.apply_gradients(zip(gradients, trainable_vars))
        self.compiled_metrics.update_state(tensors['e_data'], pred)
    ...
    def test_step(self, tensors):
        ...
        nbatch = tf.reduce_max(ind)+1
        pred = tf.math.unsorted_segment_sum(pred, ind[:, 0], nbatch)
        self.compiled_loss(tensors['e_data'], pred, regularization_losses=self.losses)
        self.compiled_metrics.update_state(tensors['e_data'], pred)
        return {m.name: m.result() for m in self.metrics}
    ...
    def call(self, tensors):
        ...
        # print(f"ind_1 shape: {tensors['ind_1'].shape}")
        # print(f"ind_2 shape: {tensors['ind_2'].shape}")
        # print(f"dist shape: {tensors['dist'].shape}")
        output = 0.0
        for i in range(self.depth):
            # prop = self.gc_blocks[i]([tensors['ind_2'], tensors['prop'], basis])
            # print(f"Before GCBlock {i}, prop shape: {tensors['prop'].shape}")
            prop = self.gc_blocks[i]([tensors['ind_2'], tensors['prop'], basis])
            # print(f"After GCBlock {i}, prop shape: {prop.shape}")

            output = self.out_layers[i]([tensors['ind_1'], prop, output])
            # tf.debugging.assert_shapes([(output, (None, self.out_layers[i].out_units))], message=f"output shape is incorrect at iteration {i}")
            if output.shape != (300,1):
                print('Wrong shape!', output.shape)
    ...
    #         prop = self.gc_blocks[i]([tensors['ind_2'], tensors['prop'], basis])
    #         output = self.out_layers[i]([tensors['ind_1'], prop, output])
    #         print("output shape at step {}: {}".format(i, output.shape))
    #         if output.shape != (300,1):
    #             print('Wrong shape!', output.shape)
    #             #     print('hello output:', output)
    #             #     sys.exit()
    #         tensors['prop'] = self.res_update[i]([tensors['prop'], prop])
    #         print("prop shape at step {}: {}".format(i, tensors['prop'].shape))
    #         print("ind_1 shape at step {}: {}".format(i, tensors['ind_1'].shape))
    #         print("ind_2 shape at step {}: {}".format(i, tensors['ind_2'].shape))
    #         print("dist shape at step {}: {}".format(i, tensors['dist'].shape))
    #         print("basis shape at step {}: {}".format(i, basis.shape))
    #     output = self.ann_output([tensors['ind_1'], output])
