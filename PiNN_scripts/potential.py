...
def potential_model(features, labels, mode, params):
    ...
    pred = tf.math.unsorted_segment_sum(pred, ind[:, 0], nbatch) #sums the batches I think

    # print('print pred2', pred)

    if mode == tf.estimator.ModeKeys.TRAIN:
        metrics = make_metrics(features, pred, model_params, mode)
        tvars = network.trainable_variables
        # print('hello metrics:', metrics)
        # print('metrics.loss:', metrics.LOSS)
        # print('metrics.error:', metrics.ERROR)
...
def make_metrics(features, pred, params, mode):
    ...
    e_pred = pred
    e_data = features['e_data'] #this is the input value (before pinet)
    if params['e_dress']:
